...
export class UdfFilterable extends UdfExpression {
  ...
  concatPredicates(predicates: UdfExpression[]) {
    predicates.forEach((p: UdfExpression) => {
      if (p instanceof UdfFilterable && p.predicates.length === 1) {
        this.predicates.push(p.predicates[0]);
      } else if (p instanceof UdfFilterable && p.type === this.type) {
        this.concatPredicates(p.predicates);
      } else {
  ...
  toQueryParam(): string {
    return this.predicates
      .map((p: UdfExpression) =>
        p instanceof UdfFilterable ? `(${p.toQueryParam()})` : p.toQueryParam()
      )
      .join(this.type === 'and' ? ';' : ',');
  }
  ...
  toArray(): any[] {
    ...
    this.predicates
      .map((p: UdfExpression) =>
        p instanceof UdfFilterable ? [p.toArray()] : p.toArray()
      )
      .forEach((p: any[]) => {
        predicates.push(p);
